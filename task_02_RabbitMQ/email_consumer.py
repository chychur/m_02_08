    ...
    def callback(ch, method, properties, body):
        ...
        if contact:
            if not contact.send_status:
                if send_email_(contact_id):
                    contact.send_status = True
                    contact.save()
                    print(f'The email was successfully sent to the "{contact.full_name}" on the email "{contact.email}"')
            else:
                print(f'The email had already sent to the "{contact.full_name}"')
        else:
            print(f"The contact with ID {contact_id} not found")

    channel.basic_consume(queue='email_route', on_message_callback=callback, auto_ack=True)
